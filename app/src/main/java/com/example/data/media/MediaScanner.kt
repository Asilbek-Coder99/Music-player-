package com.example.data.media

import android.content.ContentUris
import android.content.Context
import android.net.Uri
import android.provider.MediaStore
import com.example.domain.model.Song
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext

class MediaScanner(private val context: Context) {

    suspend fun scanLocalMedia(): List<Song> = withContext(Dispatchers.IO) {
        val songs = mutableListOf<Song>()
        try {
            val collection = MediaStore.Audio.Media.EXTERNAL_CONTENT_URI

            val projection = arrayOf(
                MediaStore.Audio.Media._ID,
                MediaStore.Audio.Media.TITLE,
                MediaStore.Audio.Media.ARTIST,
                MediaStore.Audio.Media.ALBUM,
                MediaStore.Audio.Media.DURATION,
                MediaStore.Audio.Media.ALBUM_ID,
                MediaStore.Audio.Media.IS_MUSIC
            )

            val selection = "${MediaStore.Audio.Media.IS_MUSIC} != 0 AND ${MediaStore.Audio.Media.DURATION} >= 1000"
            val sortOrder = "${MediaStore.Audio.Media.TITLE} ASC"

            context.contentResolver.query(
                collection,
                projection,
                selection,
                null,
                sortOrder
            )?.use { cursor ->
                val idColumn = cursor.getColumnIndex(MediaStore.Audio.Media._ID)
                val titleColumn = cursor.getColumnIndex(MediaStore.Audio.Media.TITLE)
                val artistColumn = cursor.getColumnIndex(MediaStore.Audio.Media.ARTIST)
                val albumColumn = cursor.getColumnIndex(MediaStore.Audio.Media.ALBUM)
                val durationColumn = cursor.getColumnIndex(MediaStore.Audio.Media.DURATION)
                val albumIdColumn = cursor.getColumnIndex(MediaStore.Audio.Media.ALBUM_ID)

                val albumArtBaseUri = Uri.parse("content://media/external/audio/albumart")

                while (cursor.moveToNext()) {
                    if (idColumn < 0) continue
                    val id = cursor.getLong(idColumn)
                    val title = if (titleColumn >= 0) cursor.getString(titleColumn) ?: "Unknown Track" else "Unknown Track"
                    val artist = if (artistColumn >= 0) cursor.getString(artistColumn) ?: "Unknown Artist" else "Unknown Artist"
                    val album = if (albumColumn >= 0) cursor.getString(albumColumn) ?: "Unknown Album" else "Unknown Album"
                    val duration = if (durationColumn >= 0) cursor.getLong(durationColumn) else 0L
                    val albumId = if (albumIdColumn >= 0) cursor.getLong(albumIdColumn) else -1L

                    val contentUri = ContentUris.withAppendedId(
                        MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, id
                    ).toString()

                    val albumArtUri = if (albumId > 0) {
                        ContentUris.withAppendedId(albumArtBaseUri, albumId).toString()
                    } else null

                    songs.add(
                        Song(
                            id = id,
                            title = title,
                            artist = if (artist == "<unknown>") "Unknown Artist" else artist,
                            album = if (album == "<unknown>") "Unknown Album" else album,
                            durationMs = duration,
                            contentUri = contentUri,
                            albumArtUri = albumArtUri
                        )
                    )
                }
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }

        if (songs.isEmpty()) {
            getSampleDemoSongs()
        } else {
            songs
        }
    }

    private fun getSampleDemoSongs(): List<Song> {
        return listOf(
            Song(
                id = -1L,
                title = "Liquid Crystal Dreams",
                artist = "Glassic Ensemble",
                album = "Translucent Horizons",
                durationMs = 214000,
                contentUri = "https://storage.googleapis.com/exoplayer-test-media-0/play.mp3",
                albumArtUri = "https://picsum.photos/seed/glassic1/500/500"
            ),
            Song(
                id = -2L,
                title = "Midnight Neon Prism",
                artist = "Synthwave Aurora",
                album = "Luminous Glass",
                durationMs = 185000,
                contentUri = "https://storage.googleapis.com/exoplayer-test-media-0/Jazz_In_Paris.mp3",
                albumArtUri = "https://picsum.photos/seed/glassic2/500/500"
            ),
            Song(
                id = -3L,
                title = "Velvet Breeze",
                artist = "Acoustic Reflections",
                album = "Soft Echoes",
                durationMs = 242000,
                contentUri = "https://storage.googleapis.com/exoplayer-test-media-1/mp3/dls/01-In_The_Mood.mp3",
                albumArtUri = "https://picsum.photos/seed/glassic3/500/500"
            ),
            Song(
                id = -4L,
                title = "Orbital Symphony",
                artist = "Cosmic Chillout",
                album = "Starlight Mirror",
                durationMs = 198000,
                contentUri = "https://storage.googleapis.com/exoplayer-test-media-0/play.mp3",
                albumArtUri = "https://picsum.photos/seed/glassic4/500/500"
            ),
            Song(
                id = -5L,
                title = "Subtle Echoes",
                artist = "Ambient Fluid",
                album = "Translucent Horizons",
                durationMs = 220000,
                contentUri = "https://storage.googleapis.com/exoplayer-test-media-0/Jazz_In_Paris.mp3",
                albumArtUri = "https://picsum.photos/seed/glassic5/500/500"
            )
        )
    }
}
